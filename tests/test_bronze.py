...
ROW_DROPPING_CALLS = (".filter(", ".where(", ".dropDuplicates(")


def test_all_expected_sources_are_declared(bronze_tables):
    assert set(bronze_tables) == EXPECTED_SOURCES


def test_table_names_are_layer_prefixed(bronze_tables):
    wrong = [name for name in bronze_tables if not name.startswith("bronze_")]
...
def test_every_table_stamps_lineage(bronze_tables, column):
    ...
    missing = [t.name for t in bronze_tables.values() if column not in t.source]
    assert not missing, f"{column} missing from: {missing}"


def test_quality_tier_is_tagged(bronze_tables):
    wrong = [t.name for t in bronze_tables.values() if t.properties.get("quality") != "bronze"]
...
def test_bronze_does_not_filter_rows(bronze_tables):
    ...
    offenders = [
        t.name for t in bronze_tables.values() if any(op in t.source for op in ROW_DROPPING_CALLS)
    ]
...
def test_bronze_reads_no_other_pipeline_table(bronze_tables):
    ...
    offenders = {t.name: t.reads for t in bronze_tables.values() if t.reads}
